...
export const searchDesignation = createAsyncThunk(
  ...
  async (searchTerm) => {
    const response = await axiosInstance.get(
      `api/setup/designation/?search=${searchTerm}`
    );
  ...
  async ({ id, name }, thunkAPI) => {
    try {
      const response = await axiosInstance.put(
        `api/setup/designation/update/${id}/`,
        { name }
      );
      return response.data.result.data;
    } catch (error) {
  ...
  async (id, thunkAPI) => {
    try {
      ...
      await axiosInstance.delete(
        `api/setup/designation/delete/${id}/`
      );
...
//     try {
//       const response = await axiosInstance.delete(`/api/designation/${id}`);
//       return id; // Return the ID of the deleted Designation
//     } catch (error) {
