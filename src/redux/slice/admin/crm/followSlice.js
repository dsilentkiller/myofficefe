...
export const fetchFollows = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axiosInstance.get("api/followup/");
  ...
  async ({ enquiryId, id }, thunkAPI) => {
    ...
    try {
      const response = await axiosInstance.get(
        `api/followup/?enquiry_id=${enquiryId}` // Assuming this returns an array of follow-ups
      );
...
export const deleteFollow = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      await axiosInstance.delete(`api/followup/delete/${id}/`);
      return id;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.response?.data || error.message);
    }
...
export const searchFollow = createAsyncThunk(
  ...
  async (searchTerm, thunkAPI) => {
    try {
      const response = await axiosInstance.get(
        `api/followup/?search=${searchTerm}`
      );
  ...
  async (enquiryId, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get(
        ...
        `api/followup/followup-by-enquiry-id/?${enquiryId}/`
      );
