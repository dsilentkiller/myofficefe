  ...
  async (enquiryId, { rejectWithValue }) => {
    try {
      ...
      const response = await axiosInstance.post(
        `api/enquiry/convert_enquiry_to_customer/${enquiryId}/`
      ...
      console.error("Convert API Error:", error?.response || error.message);
      return rejectWithValue(
        error?.response?.data?.message || "Error converting enquiry to customer"
      );
    }
  }
);
...
export const updateEnquiryStatus = createAsyncThunk(
  ...
  async ({ id, status }) => {
    const response = await axiosInstance.put(`api/enquiry/update/${id}/`, {
      status,
    });
    return response.data.result;
  }
...
export const fetchEnquiryById = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await axiosInstance.get(`api/enquiry/detail/${id}/`);
...
export const fetchEnquiryByIdUpdate = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await axiosInstance.get(`api/enquiry/update/${id}/`);
      console.log("API Response:", response.data); // Log to check the structure of the response
      return response.data.result.enquiry; // Ensure this is the correct path
    } catch (error) {
      return thunkAPI.rejectWithValue(error.response?.data || error.message);
  ...
  async (formData, { rejectWithValue }) => {
    try {
      ...
      const response = await axiosInstance.put(
        `api/enquiry/update/${formData.id}/`,
        formData
      );
...
export const fetchLostEnquiries = createAsyncThunk(
  ...
  async () => {
    const response = await axiosInstance.get("api/enquiry/lost/");
    return response.data.result;
  }
);
