  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get(
        "http://127.0.0.1:8000/api/cms/request-demo/"
      );
  ...
  async (formData, thunkAPI) => {
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/cms/request-demo/create/",
        formData
      );
      return response.data.result;
    } catch (error) {
...
export const fetchDemoById = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/cms/request-demo/update/${id}/`
      );
...
export const updateDemoStatus = createAsyncThunk(
  ...
  async ({ id, status }) => {
    const response = await axios.put(
      `http://127.0.0.1:8000/api/cms/request-demo/${id}/`,
      { status }
    );
    return response.data.result;
  }
...
export const deleteDemo = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      await axios.delete(
        `http://127.0.0.1:8000/api/cms/request-demo/delete/${id}/`
      );
      return id;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.response?.data || error.message);
...
export const searchDemo = createAsyncThunk(
  ...
  async (searchTerm, thunkAPI) => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/cms/request-demo/?search=${searchTerm}`
      );
