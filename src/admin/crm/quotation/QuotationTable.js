...
const QuotationTable = () => {
  ...
  const handleRowAction = (actionKey, rowData) => {
    if (actionKey === "edit") {
      navigate(`/dashboard/crm/quotation/update/${rowData.id}`);
      // navigate(`/dashboard/crm/quotation/${activeTable}-quotation/update/${rowData.id}`);
    } else if (actionKey === "delete") {
      // Handle delete logic
    } else if (actionKey === "view") {
      navigate(`/dashboard/crm/quotation/detail/${rowData.id}`);
    }
  };
  ...
  return (
    <div className="content-wrapper col-md-11 col-ms-11" style={{ display: "flex", justifyContent: "center", flexDirection: "column", overflow: "hidden" }}>
      <AppBar position="static" color="gray">
        <Toolbar>
          ...
          <Button onClick={exportToPDF} startIcon={<PdfIcon />} variant="contained" sx={{ marginLeft: 2 }}>
            {/* Export PDF */}
          </Button>
          <CSVLink
            data={filteredQuotations}
            headers={[
              { label: "Enquiry Name", key: "quotation..enquiry_name" },
          ...
          >
            <Button startIcon={<CsvIcon />} variant="contained" sx={{ marginLeft: 2 }}>
              {/* Export CSV */}
            </Button>
          </CSVLink>
        </Toolbar>
      ...
      {/* Table Container */}
      <TableContainer component={Paper} sx={{ maxHeight: 800, overflowX: "auto", marginTop: 2, border: "1px solid #ccc", flexGrow: 1, overflowY: "auto" }}>
...
//   // Filter quotations based on search term
//   const filteredQuotations = (activeTable === "product" ? formattedProductQuotations : formattedServiceQuotations).filter((quotation) => {
//     // Ensure that `enquiry_name` is a string and handle potential null or undefined values
//     const enquiryName = String(quotation.enquiry_name || "").toLowerCase(); // Convert to string and handle null/undefined
//     const lowerCaseSearchTerm = searchTerm.toLowerCase();
