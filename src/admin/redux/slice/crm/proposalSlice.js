...
const API_BASE_URL = 'http://127.0.0.1:8000/api/proposal/';
...
export const fetchProposals = createAsyncThunk('proposal/fetchProposals', async (_, { rejectWithValue }) => {
  try {
    const response = await axios.get('http://127.0.0.1:8000/api/proposal/');
...
export const createProposal = createAsyncThunk(
  ...
  async (formData, { rejectWithValue }) => {
    try {
      const response = await axios.post('http://127.0.0.1:8000/api/proposal/create/', formData);
...
const proposalSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(deleteProposal.fulfilled, (state, action) => {
      state.proposals = state.proposals.filter((proposal) => proposal.id !== action.payload);
