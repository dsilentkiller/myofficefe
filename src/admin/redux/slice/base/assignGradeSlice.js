...
export const searchAssignGrade = createAsyncThunk(
  ...
  async (searchTerm) => {
    const response = await axios.get(
      `http://127.0.0.1:8000/api/setup/assign-grade/?search=${searchTerm}`
    );
...
export const fetchAssignGrade = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get(
        "http://127.0.0.1:8000/api/setup/assign-grade/"
      );
  ...
  async (municipalityData, thunkAPI) => {
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/setup/assign-grade/create/",
        municipalityData
      );
      return response.data.result.data; // Adjust this based on your actual API response structure
    } catch (error) {
...
export const fetchAssignGradeById = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/setup/assign-grade/${id}/`
      );
      return response.data.result.data; // Adjust this based on your actual API response structure
    } catch (error) {
      return thunkAPI.rejectWithValue(error.response.data);
  ...
  async ({ id, name }, thunkAPI) => {
    try {
      const response = await axios.put(
        `http://127.0.0.1:8000/api/setup/assign-grade/update/${id}/`,
        { name }
      );
      return response.data.result.data;
    } catch (error) {
...
export const deleteMunicipality = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      ...
      await axios.delete(
        `http://127.0.0.1:8000/api/setup/assign-grade/delete/${id}/`
      );
