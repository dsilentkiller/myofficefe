...
const CustomerForm = () => {
  ...
  useEffect(() => {
    if (id && customerToUpdate) {
      // Populate form data when the customer is being updated
      setFormData(customerToUpdate);
    } else {
      // Reset form data when creating a new customer
      setFormData({
        customer_name: "",
        customer_type: "",
        pri_phone: "",
  ...
  }, [id, customerToUpdate]); // Dependency array to only trigger this when the `id` or `customerToUpdate` changes.
  ...
  useEffect(() => {
    console.log("Current Enquiry State:", customerToUpdate);
  }, [customerToUpdate]);
  ...
  useEffect(() => {
    if (formData) {
      setFormData(formData); // Set form data when formData is available
    }
  }, [formData]);
  // useEffect(() => {
...
// Fetch data when component mounts
  ...
  const handleSubmit = (e) => {
    ...
    if (id) {
      dispatch(updateCustomer({ id, ...formDataToSubmit }))
        .unwrap()
        ...
        .catch((error) => {
          ...
          toast.error(
            `Failed to update customer: ${error.message || "An error occurred"}`
          );
        });
    } else {
      dispatch(createCustomer(formDataToSubmit))
        ...
        .catch((error) => {
          console.error("API error:", error);
          toast.error(
            `Failed to create customer: ${error.message || "An error occurred"}`
          );
        });
    }
  };
...
// const formData = useSelector((state) => state.customers.currentEnquiry);
